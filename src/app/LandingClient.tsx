                        ...
                        {session ? (
                            <>
                                ...
                                {(session.user as any)?.role === 'ARTIST' && (
                                    <Link href="/artist" className="btn btn-secondary" style={{ padding: '0.5rem 1rem', fontSize: '0.875rem', background: 'rgba(13, 242, 89, 0.1)', color: 'var(--primary-color)', border: '1px solid rgba(13, 242, 89, 0.2)' }}>
                                        Studio
                                    </Link>
                                )}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                    <span style={{ fontSize: '2.25rem', fontWeight: 700, color: '#fff' }}>{heroTrack.priceStream} FCFA</span>
                        ...
                        <div style={{ position: 'relative' }}>
                            <div className="glass-panel" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '2rem', borderRadius: '1.5rem' }}>
                                <div style={{ aspectRatio: '1', width: '100%', borderRadius: '1rem', overflow: 'hidden', position: 'relative' }}>
                                    {heroTrack.coverImage && <img alt="Audio Artwork" style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'blur(4px)', transform: 'scale(1.05)' }} src={heroTrack.coverImage} />}
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                        <div>
                                            <h3 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#fff', margin: 0 }}>{heroTrack.title}</h3>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '1.5rem' }}>
                                {initialTracks.map(track => (
                                    <div key={track.id} className="glass-panel" style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem', cursor: 'pointer', borderRadius: '1rem' }} onClick={() => { setSelectedTrack(track); setIsModalOpen(true); }}>
                                        <div style={{ aspectRatio: '1', borderRadius: '0.75rem', overflow: 'hidden', position: 'relative' }}>
                                            {track.coverImage ? (
                                                <img alt={track.title} src={track.coverImage} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'blur(8px)' }} />
                                            ) : (
                                        ...
                                        <div>
                                            <h4 style={{ color: '#fff', fontWeight: 700, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{track.title}</h4>
                                            <p style={{ color: 'var(--primary-color)', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', marginTop: '0.25rem', marginBottom: 0 }}>{track.artistName || track.artist?.name}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
